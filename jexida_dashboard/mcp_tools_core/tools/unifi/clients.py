...
async def unifi_list_clients(params: UniFiListClientsInput) -> UniFiListClientsOutput:
    ...
    """
    logger.info(f"unifi_list_clients called with site_id={params.site_id}, wifi_only={params.wifi_only}")
    ...
    try:
        async with UniFiClient(site=params.site_id) as client:
            ...
            logger.info(f"Found {len(client_list)} clients ({len(wifi_clients)} WiFi, {len(wired_clients)} wired)")
