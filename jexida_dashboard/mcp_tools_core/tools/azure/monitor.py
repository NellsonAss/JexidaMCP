...
Provides the http_health_probe tool for checking endpoint health.
"""
...
# Default settings
HTTP_PROBE_DEFAULT_TIMEOUT = int(os.environ.get("HTTP_PROBE_DEFAULT_TIMEOUT", "30"))
HTTP_PROBE_MAX_TIMEOUT = int(os.environ.get("HTTP_PROBE_MAX_TIMEOUT", "120"))


class HttpHealthProbeInput(BaseModel):
    """Input schema for http_health_probe tool."""
...
class HttpHealthProbeOutput(BaseModel):
    """Output schema for http_health_probe tool."""
...
async def http_health_probe(params: HttpHealthProbeInput) -> HttpHealthProbeOutput:
    ...
    """
    logger.info(
        f"http_health_probe called: url={params.url}, method={params.method}, "
        f"expected_status={params.expected_status}"
    )

    # Determine timeout
    timeout = params.timeout_seconds
    if timeout is None:
        timeout = HTTP_PROBE_DEFAULT_TIMEOUT
    else:
        # Cap at maximum
        timeout = min(timeout, HTTP_PROBE_MAX_TIMEOUT)
    ...
    try:
        ...
        if is_healthy:
            logger.info(f"Health check passed: status={response.status_code}, time={elapsed_ms}ms")
        else:
            logger.warning(
                f"Health check failed: expected={params.expected_status}, "
                f"got={response.status_code}, time={elapsed_ms}ms"
            )
        ...
        logger.error(f"Health check timeout: {error_msg}")
        ...
        logger.error(f"Health check connection error: {error_msg}")
        ...
        logger.error(f"Health check request error: {error_msg}")
        ...
        logger.exception(f"Health check unexpected error: {error_msg}")
