...
class Config:
    ...
    def load(self) -> None:
        ...
        self.data = {
            ...
            "model": {
                "name": "phi3",
                "mode": "direct",  # direct, cascade, route, orchestrate
            },
            "mcp": {
                "port": 8080,
                "timeout": 60.0,
    ...
    def _merge_config(self, file_data: Any) -> None:
    ...
    def model_mode(self) -> str:
        """Get the current model mode (direct, cascade, route, orchestrate)."""
        return self.data["model"].get("mode", "direct")
    ...
    def set_model_mode(self, mode: str) -> None:
        """Set the model mode and save to config file.
        
        Args:
            mode: The new mode (direct, cascade, route, orchestrate)
        """
        valid_modes = ("direct", "cascade", "route", "orchestrate")
        if mode not in valid_modes:
            raise ValueError(f"Invalid mode: {mode}. Must be one of {valid_modes}")
        self.data["model"]["mode"] = mode
        self.save()
    ...
    def is_whitelisted(self, command: str) -> bool:
        ...
        - Glob/wildcard: "cat *" matches "cat foo.txt"
        - Regex (prefix with ~): "~docker.*" matches "docker ps"
